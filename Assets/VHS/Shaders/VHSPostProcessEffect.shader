	...
	SubShader {
		Pass {
			...
			fixed4 frag (v2f_img i) : COLOR{
				...
				// Проверяем, готова ли VHS текстура
				// Если красный канал значительно больше других - текстура не готова
				float vhsSum = vhs.r + vhs.g + vhs.b;
				bool vhsValid = vhsSum > 0.01 && (vhs.r < vhs.g + vhs.b + 0.5);
				...
				// Если VHS текстура не готова, возвращаем оригинальное изображение
				if (!vhsValid) {
					return c;
				}
				
				// Нейтрализуем избыточный красный оттенок в VHS текстуре
				float avgGB = (vhs.g + vhs.b) * 0.5;
				if (vhs.r > avgGB + 0.2) {
					vhs.r = avgGB;
				}
				...
				// Убрали добавление красного - теперь bleed влияет на яркость, а не на красный канал
				if(bleed > 0.1){
					float bleedAmount = bleed * _xScanline * 0.3;
					vhs += fixed4(bleedAmount, bleedAmount, bleedAmount, 0);
				}
				...
				// Ослабляем VHS эффект, чтобы убрать красные полосы
				vhs *= 0.5;
